...
def main():
    ...
    sys.stdout = Logger(os.path.join('logs', 'amal_%s_%s.txt' % (opts.model,'22')))  # Log存入对应名称文档
    print(opts)
    ...
    mkdir_if_missing('checkpoints')
    latest_ckpt = 'checkpoints/amal_%s_latest_%s.pth' % (opts.model,'22')  # 存入最终结果

    # latest_ckpt = 'checkpoints/epoch8b4b2bp03beta07l6t507792/amal_resnet34_epoch_40.pth'   # 存入最终结果
    best_ckpt = 'checkpoints/amal_%s_best_%s.pth' % (opts.model,'22')  # 存入最好结果
    ...
    t1 = _model_dict[t1_model_name](num_classes=210).to(device)  # cub200
    t2_model_name = opts.t2_ckpt.split('/')[1].split('_')[1]
    t2 = _model_dict[t2_model_name](num_classes=130).to(device)  # dogs
    ...
    while cur_epoch < opts.epochs:
        ...
        cur_epoch += 1
        if cur_epoch % 5==0:
            ckpt_name = 'checkpoints/epoch22/amal_{0}_epoch_{1}.pth'.format(opts.model, cur_epoch)
            save_ckpt(ckpt_name)
