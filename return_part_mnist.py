...
if __name__ == '__main__':
    x_train, y_train, x_test, y_test = return_whole_mnist()
    x_train, y_train, x_test, y_test = return_part_mnist([0,1, 2,3])#需要的label
    # 将一个矩阵保存为图片
    root = '/media/data/wlanyu/cfl/CommonFeatureLearning-master/data/mnist'
    ct = 0
    x_train_lst = x_train.tolist()
    y_train_lst = y_train.tolist()
    for img,target in zip(x_train_lst,y_train_lst):
        save_path = os.path.join(root,folder,'image','%s.jpg'%(ct))
        img = np.asarray(img)
        img = np.squeeze(img,axis=2)
        im=Image.fromarray(img.astype(np.uint8), mode='L')
        im.save(save_path)
        f = open(os.path.join(root,folder,'label.txt'),'a')
        f.write(str(ct)+' '+str(target)+'\n')
        f.close()
        f=open(os.path.join(root,folder,'image.txt'),'a')
        f.write(str(ct)+' '+'%s.jpg'%(ct)+'\n')
        f.close()
        ct += 1
        is_train_img = np.random.binomial(1,train_set_pro,size=None)
        f=open(os.path.join(root,folder,'train_test_split.txt'),'a')
        f.write(str(ct)+' '+str(is_train_img)+'\n')
        f.close()
