...
def get_parser():
    ...
    parser.add_argument("--t4_ckpt", type=str, default='checkpoints/dogs_resnet34_best.pth')
...
def amal(cur_epoch, criterion_ce, criterion_cf, model, cfl_blk,cam, teachers, optim, train_loader, device, scheduler=None,
    ...
    for cur_step, (images, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            t3_out = t3(images)
            t4_out = t4(images)
            t_outs = torch.cat((t1_out, t2_out,t3_out,t4_out), dim=1)#concat 两个类的作为标签。
...
def main():
    ...
    sys.stdout = Logger(os.path.join('logs', 'amal_%s_%s.txt' % (opts.model,'4torigin')))  # Log存入对应名称文档
    print(opts)
    ...
    mkdir_if_missing('checkpoints')
    latest_ckpt = 'checkpoints/amal_%s_latest_%s.pth' % (opts.model,'4torigin')  # 存入最终结果

    # latest_ckpt = 'checkpoints/epoch8b4b2bp03beta07l6t507792/amal_resnet34_epoch_40.pth'   # 存入最终结果
    best_ckpt = 'checkpoints/amal_%s_best_%s.pth' % (opts.model,'4torigin')  # 存入最好结果
    ...
    t4_model_name = opts.t4_ckpt.split('/')[1].split('_')[1]
    t4 = _model_dict[t4_model_name](num_classes=120).to(device)  # dogs
    ...
    cfl_blk = CFL_ConvBlock(stu_feature_dim, [t1_feature_dim, t2_feature_dim,t3_feature_dim, t4_feature_dim], 128).to(
        device)  ##################################
    cam = CAM().to(device)
    ...
    while cur_epoch < opts.epochs:
        ...
        cur_epoch += 1
        if cur_epoch % 5==0:
            ckpt_name = 'checkpoints/epochorigin/amal_{0}_epoch_{1}_%s.pth'.format(opts.model, cur_epoch,'4tcfl')
            save_ckpt(ckpt_name)
