...
class CAM(nn.Module):
    ...
    def forward(self, f1_, f2_):
        f1 = torch.stack(f1_).transpose(0,1)
        f2 = f2_.unsqueeze(1)
        b, n1, c, h, w = f1.size()
        n2 = f2.size(1)
        ...
        f1=f1.mean(2).squeeze(2)
#        f2=f2.mean(1).squeeze(1)
        f2 = f2.squeeze(2)#b n1 c h w
        f1 = f1.transpose(0,1)
        f1_lst = []
        for elm in f1:
            f1_lst.append(elm)
        f2 = f2.transpose(0,1)#n1 b c h w
        f2_lst = []
        for elm in f2:
            f2_lst.append(elm)#b c h w
        return f1_lst, f2_lst