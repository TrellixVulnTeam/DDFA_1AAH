...
#
#     def __init__(self, root, split='train', download=False, transforms=None, offset=0,labeloffset=0):
...
#         files = [str(mat_file['file_list'][i][0][0]) for i in range(size)]
#         labels = np.array(
#             [mat_file['labels'][i][0]-1 for i in range(size)])
#         self.files = []
#         self.labels = []
#         for i in range(len(labels)):
#             if labels[i] >=labeloffset and labels[i]<=labeloffset+29:
#                 self.files.append(files[i])
#                 self.labels.append(labels[i]-labeloffset)
