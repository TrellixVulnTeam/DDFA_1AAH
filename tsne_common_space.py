...
def get_parser():
    ...
    # parser.add_argument("--ckpt", type=str, default='amal_resnet34_best.pth')
    parser.add_argument("--t1_ckpt", type=str, default='cub200_resnet18_best.pth')
    parser.add_argument("--t2_ckpt", type=str, default='mnist_resnet18_0_9_best_997164.pth')
    parser.add_argument("--t3_ckpt", type=str, default='USPS_resnet18_0_9_best_993534.pth')




    # parser.add_argument("--ckpt", type=str, required=True, default='amal_resnet34_best.pth')
    # parser.add_argument("--t1_ckpt", type=str, required=True, default='cub200_resnet18_best.pth')
    # parser.add_argument("--t2_ckpt", type=str, required=True, default='dogs_resnet34_best.pth')
...
def get_samples(loader, classes, sample_num=100):
...
def main():
    ...
    ckpt_list = os.listdir(os.path.join(ckpt_dir,'epoch3origin'))
    for ckpt_name in ckpt_list:
        t1_ckpt = os.path.join(ckpt_dir, opts.t1_ckpt)
        t2_ckpt = os.path.join(ckpt_dir, opts.t2_ckpt)
        t3_ckpt = os.path.join(ckpt_dir, opts.t3_ckpt)
        stu_ckpt = os.path.join(ckpt_dir, 'epoch3origin',ckpt_name)

        t1_model_name = t1_ckpt.split('/')[2].split('_')[1]
        t2_model_name = t2_ckpt.split('/')[2].split('_')[1]
        t3_model_name = t3_ckpt.split('/')[2].split('_')[1]
        ...
        t2 = _model_dict[t2_model_name](num_classes=10).to(device)
        t3 = _model_dict[t3_model_name](num_classes=10).to(device)
        ...
        t3.load_state_dict(torch.load(t3_ckpt, map_location=lambda storage, loc: storage)['model_state'])
        ...
        t3_feature_dim = t3.fc.in_features
        ...
        tsne_on_features_space = True if ( t1_feature_dim==t2_feature_dim and t2_feature_dim==t3_feature_dim and t3_feature_dim==stu_feature_dim) else False
        cfl_blk = CFL_ConvBlock(stu_feature_dim, [t1_feature_dim, t2_feature_dim,t3_feature_dim], 128).to(device)
        ...
        with torch.no_grad():
            ...
            for j in range(11):
                ...
                class_list = np.arange(j, num_classes, 11) # (120+200) // 20 classes = 16 (interval)
                # class_list = np.arange(0, num_classes)  # (120+200) // 20 classes = 16 (interval)
                cmap = matplotlib.cm.get_cmap('tab20')
                # TODO: make it fast.
                images, labels = get_samples(val_loader, class_list, 10)
                ...
                for img, lbl in tqdm(zip(images, labels)):
                    ...
                    _ft3 = t3.layer4.output
                    if is_densenet:
                        _fs = stu.features.output
                    else:
                        _fs = stu.layer4.output
                    (_hs, _ht), (_, _) = cfl_blk(_fs, [_ft1, _ft2,_ft3])
                ...
                ft3 = torch.cat(ft3, dim=0)
                fs = torch.cat(fs, dim=0)
                hs = torch.cat(hs, dim=0)
                ht = [ torch.cat(ht1, dim=0), torch.cat(ht2, dim=0),torch.cat(ht3, dim=0) ]
                ...
                features = F.normalize( torch.cat( features, dim=0 ), p=2, dim=1 ).view(4*N, C, -1).mean(dim=2).cpu().numpy()
                ...
                for i in range(3): # 2 classification tasks
                ...
                plt.show()
                plt.savefig('tsne_results/%s/epoch_%s_common_space_tsne_%d.png'%(stu_model_name,stu_ckpt.split('_')[3].split('.')[0],j))
                plt.close()

                # ========= Draw features space =========
                if tsne_on_features_space:
                    features = [ fs, ft1, ft2 ,ft3]
                    features = F.normalize( torch.cat( features, dim=0 ), p=2, dim=1 ).view(4*N, C, -1).mean(dim=2).cpu().numpy()
        ...
        #
                    ...
                    ax.legend(fontsize="xx-large", markerscale=2)
                    plt.show()
                    # plt.savefig('tsne_results/%s/feature_space_tsne_%d.svg'%(stu_model_name, j))
